...
import { 
  ...
  Copy, ChevronRight, Navigation, Trash2, ArrowLeft, PackageCheck, Info
...
const handlePayment = async () => {
  if (!customer.value.nome || !customer.value.cpf || shippingValue.value === 0) {
  ...
  try {
    const res = await fetch(`${API_URL}/produtos/checkout/pix`, {
      ...
      body: JSON.stringify({
        ...
        email: customer.value.email || 'vendas@loja.com',
        cpf: customer.value.cpf,
        valor: totalFinal.value
      })
    })
    const data = await res.json()
    if (data.success) pixData.value = data
    else alert("Erro Asaas: " + (data.error || "Chave Inválida"))
  } catch (e) {
  ...
  <div class="max-w-md mx-auto bg-slate-50 min-h-screen pb-24 font-sans text-slate-900 overflow-x-hidden relative">
    ...
    <header class="sticky top-0 z-50 bg-white/90 backdrop-blur-lg p-4 flex justify-between items-center border-b border-slate-100 shadow-sm">
      <div class="flex items-center gap-2">
        ...
        <h1 class="font-black text-2xl tracking-tighter italic text-indigo-600">TudoPassa Store</h1>
      </div>
    ...
    <div v-if="currentStep === 'details' && selectedProduct" class="animate-in slide-in-from-bottom-20">
      ...
      <div class="p-8 bg-white rounded-t-[3.5rem] -mt-16 relative z-10 shadow-2xl">
        ...
        <button @click="handleAddToCart" class="w-full bg-slate-900 py-6 rounded-[2rem] text-white font-black text-lg shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3">
          <ShoppingBag class="w-6 h-6" /> ADICIONAR À CESTA
        </button>
      </div>
    </div>
    ...
    <!-- STEP 4: CHECKOUT (SUPERFRETE + PIX) -->
    <div v-if="currentStep === 'checkout'" class="p-6 animate-in slide-in-from-right">
        ...
        <div class="space-y-4 mb-8">
            <input v-model="customer.nome" placeholder="Nome Completo" class="w-full p-5 bg-white rounded-2xl border-none shadow-sm outline-none focus:ring-2 ring-indigo-500/20">
            <input v-model="customer.cpf" placeholder="CPF" class="w-full p-5 bg-white rounded-2xl border-none shadow-sm outline-none focus:ring-2 ring-indigo-500/20">
            <input v-model="customer.endereco" @blur="calculateShipping" placeholder="Endereço Completo de Entrega" class="w-full p-5 bg-white rounded-2xl border-none shadow-sm outline-none focus:ring-2 ring-indigo-500/20">
        </div>

        <!-- TAXA DE ENTREGA (SUPERFRETE) -->
        <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 mb-8">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-2">
                    <Truck class="w-4 h-4 text-indigo-600" />
                    <span class="text-xs font-black uppercase tracking-widest text-slate-400">Entrega (SuperFrete)</span>
                </div>
            ...
            <div class="flex items-center gap-2 p-3 bg-slate-50 rounded-xl">
                ...
                <p class="text-[9px] text-slate-400 font-bold uppercase leading-tight">O valor do frete é calculado automaticamente com base no seu endereço.</p>
            </div>
        </div>

        <div class="bg-indigo-600 p-8 rounded-[3rem] text-white mb-8 shadow-xl shadow-indigo-100">
    ...
    <div v-if="currentStep === 'tracking'" class="p-6 animate-in slide-in-from-bottom-20 duration-500">
        <div class="flex items-center gap-5 mb-10">
            ...
            <div>
                ...
                <p class="text-slate-400 font-bold text-[10px] uppercase tracking-widest">Acompanhe sua jornada</p>
            </div>
        </div>
        ...
        <div class="bg-slate-900 h-80 rounded-[3rem] overflow-hidden relative shadow-2xl border-8 border-white">
            <div id="map" class="w-full h-full bg-slate-200 flex flex-col items-center justify-center">
                ...
                <p class="text-slate-400 font-black text-[9px] uppercase">Localizando entregador...</p>
            </div>
