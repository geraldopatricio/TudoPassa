...
import { 
  ...
  Copy, ChevronRight, Navigation, Trash2, ArrowLeft, PackageCheck, Info, Layers
...
const isGridMode = ref(false) // NOVO: Controle da Grade
...
const openDetails = (p) => {
  ...
  selectedQty.value = 1
  isGridMode.value = false // Reseta o modo grade ao abrir novo produto
...
const handleAddToCart = () => {
  ...
  if (isGridMode.value) {
    // Modo Grade: Limpa apenas o tamanho e quantidade para escolher o próximo
    selectedSize.value = null
    selectedQty.value = 1
  } else {
    // Modo Normal: Vai para o carrinho
...
const handlePayment = async () => {
  if (!customer.value.nome || !customer.value.cpf || !customer.value.email || shippingValue.value === 0) {
    alert("Preencha todos os dados (incluindo email) e aguarde o frete!")
    return
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      // 2. Envia e-mail de notificação (Frontend chama a nova rota do backend)
  ...
  <div class="max-w-md mx-auto bg-slate-50 min-h-screen pb-24 font-sans text-slate-900 overflow-x-hidden relative">
    ...
    <main v-if="currentStep === 'home'" class="p-4 animate-in fade-in">
      ...
      <div v-if="loading" class="flex flex-col items-center py-20 gap-4">
        ...
        <p class="text-slate-400 font-bold text-[10px] tracking-widest uppercase">Atualizando...</p>
      </div>

      <div v-else>
        <div class="grid grid-cols-2 gap-4">
          <div v-for="p in paginatedProducts" :key="p.referencia" @click="openDetails(p)" 
               class="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-slate-100 active:scale-95 transition-all">
            <img :src="`${API_URL}/uploads/${p.imagem}`" class="w-full h-48 object-cover">
            <div class="p-4">
    ...
    <div v-if="currentStep === 'details' && selectedProduct" class="animate-in slide-in-from-bottom-20">
      <div class="relative h-[45vh]">
        <img :src="`${API_URL}/uploads/${selectedProduct.imagem}`" class="w-full h-full object-cover">
        <button @click="currentStep = 'home'" class="absolute top-4 left-4 p-3 bg-white/80 backdrop-blur rounded-full"><ArrowLeft/></button>
      </div>
      ...
      <div class="p-8 bg-white rounded-t-[3.5rem] -mt-16 relative z-10 shadow-2xl">
        ...
        <!-- NOVO: COMPRAR EM GRADE TOGGLE -->
        <div class="flex items-center justify-between bg-indigo-50 p-4 rounded-2xl mb-8 border border-indigo-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[9px] text-indigo-500 font-bold">Adicione vários tamanhos</p>
                </div>
            </div>
            <button @click="isGridMode = !isGridMode" 
                ...
                <div :class="isGridMode ? 'translate-x-6' : 'translate-x-1'" 
                     class="absolute top-1 w-4 h-4 bg-white rounded-full transition-transform duration-300"></div>
            </button>
        </div>

        <h4 class="font-black text-[10px] uppercase tracking-widest mb-4">Selecione o Tamanho</h4>
        ...
        <h4 class="font-black text-[10px] uppercase tracking-widest mb-4">Quantidade</h4>
    ...
    <div v-if="currentStep === 'cart-summary'" class="p-6 animate-in fade-in">
        <div class="flex justify-between items-end mb-8">
            ...
            <button @click="currentStep = 'home'" class="text-indigo-600 font-bold text-xs uppercase underline tracking-widest">Adicionar mais</button>
        </div>
    ...
    <div v-if="currentStep === 'checkout'" class="p-6 animate-in slide-in-from-right">
        ...
        <div class="space-y-4 mb-8">
            ...
            <input v-model="customer.email" placeholder="E-mail (Para receber o pedido)" class="w-full p-5 bg-white rounded-2xl border-none shadow-sm outline-none">
            <input v-model="customer.cpf" placeholder="CPF" class="w-full p-5 bg-white rounded-2xl border-none shadow-sm outline-none">
            <input v-model="customer.whatsapp" placeholder="WhatsApp" class="w-full p-5 bg-white rounded-2xl border-none shadow-sm outline-none">
            <input v-model="customer.endereco" @blur="calculateShipping" placeholder="Endereço de Entrega" class="w-full p-5 bg-white rounded-2xl border-none shadow-sm outline-none">
        </div>

        <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 mb-8">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-2"><Truck class="w-4 h-4 text-indigo-600" /> <span class="text-xs font-black uppercase text-slate-400">Entrega</span></div>
        ...
        <div class="bg-indigo-600 p-8 rounded-[3rem] text-white mb-8 shadow-xl">
            <div class="flex justify-between items-center mb-6">
                ...
                <span class="text-3xl font-black">R$ {{ totalFinal.toFixed(2) }}</span>
            </div>
            ...
            <div v-if="pixData" class="bg-white p-6 rounded-3xl text-center mb-6 animate-in zoom-in">
                ...
                <button @click="finishAndTrack" class="w-full bg-emerald-500 text-white py-4 rounded-xl font-black">JÁ PAGUEI / RASTREAR</button>
            </div>
            <button v-else @click="handlePayment" :disabled="calculatingShipping || shippingValue === 0" class="w-full bg-white text-indigo-600 py-5 rounded-2xl font-black uppercase tracking-widest disabled:opacity-50">
                GERAR PIX AGORA
            </button>
        </div>
    </div>
    ...
    <!-- STEP 5: TRACKING -->
    <div v-if="currentStep === 'tracking'" class="p-6 animate-in slide-in-from-bottom-20">
        <div class="flex items-center gap-5 mb-10">
            <div class="w-16 h-16 bg-emerald-500 text-white rounded-3xl flex items-center justify-center shadow-lg"><CheckCircle2 class="w-8 h-8" /></div>
            <div><h2 class="font-black text-2xl tracking-tighter">Pedido Realizado!</h2><p class="text-slate-400 font-bold text-[10px] uppercase">Enviamos um e-mail com os detalhes.</p></div>
        </div>
        <div id="map" class="w-full h-80 bg-slate-200 rounded-[3rem] border-8 border-white shadow-2xl"></div>
        <button @click="currentStep = 'home'" class="w-full mt-10 py-6 text-slate-400 font-black text-[10px] uppercase border-2 border-slate-200 rounded-[2rem]">Voltar para a loja</button>
    </div>

    <!-- BOTTOM NAV (APENAS HOME) -->
    <nav v-if="currentStep === 'home'" class="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-slate-100 p-5 flex justify-around items-center z-[90]">
        ...
        <div class="bg-indigo-600 w-16 h-16 rounded-full -mt-14 border-[6px] border-slate-50 flex items-center justify-center text-white shadow-xl shadow-indigo-100"><Plus class="w-8 h-8" /></div>
